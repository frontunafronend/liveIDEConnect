...
let chatPanel = null;
let messageQueue = [];
let pendingChatMessages = []; // Messages waiting to be shown (kept for backward compatibility)
let tabId = (0, crypto_1.randomUUID)(); // Unique ID for this Cursor tab/window
let chatParticipant; // Chat participant for official API integration (using any since types may not be fully available)
let latestWebAppMessage = null; // Store latest message for chat participant
function getConfig() {
    const config = vscode.workspace.getConfiguration('liveideconnect');
    return {
        token: config.get('token') || '',
        sessionId: config.get('sessionId') || '',
        apiBase: config.get('apiBase', 'http://localhost:4000'),
        autoSendToCursorChat: config.get('autoSendToCursorChat', true)
    };
}
...
function connectWebSocket() {
    ...
    if (token.includes('your-token') || token.includes('example') || token.length < 20) {
        vscode.window.showErrorMessage('LiveIDEConnect: Invalid token detected. Please set a valid token in settings.', 'Open Settings').then(selection => {
    ...
    if (sessionId.includes('your-session') || sessionId.includes('example') || sessionId.length < 10) {
        vscode.window.showErrorMessage('LiveIDEConnect: Invalid sessionId detected. Please set a valid sessionId in settings.', 'Open Settings').then(selection => {
            if (selection === 'Open Settings') {
    ...
    ws.on('error', (error) => {
        ...
        vscode.window.showErrorMessage(`LiveIDEConnect Error: ${errorMsg}. Check the "LiveIDEConnect" output channel for details.`, 'Open Output Channel').then(selection => {
    ...
    ws.on('close', (code, reason) => {
        ...
        if (isAuthError) {
            ...
            vscode.window.showErrorMessage(`LiveIDEConnect: Authentication failed (Code: ${code}). ` +
                `Please verify your token and sessionId in settings. ` +
                `Reason: ${reasonStr}`, 'Open Settings', 'Open Output Channel').then(selection => {
                if (selection === 'Open Settings') {
 ...
 */
async function sendMessageToCursorChat(messageContent) {
    try {
        ...
        try {
            ...
            const submitChoice = await vscode.window.showInformationMessage(`Message typed into Cursor chat. Submit it?`, 'Submit', 'Skip');
 ...
 */
async function sendMessageWithCursorIntegration(messageContent) {
    ...
    if (autoSendToCursorChat) {
        ...
        const choice = await vscode.window.showInformationMessage(`💬 Message sent to web app. Also send to Cursor chat?`, 'Send to Cursor Chat', 'Copy to Clipboard', 'Skip');
 ...
 */
function registerChatVariables(context) {
    if (!vscode.chat) {
        return;
    }
    try {
        ...
        if (chatApi.createChatVariable) {
            const chatVariable = chatApi.createChatVariable('liveideconnect_message', 'Latest message from LiveIDEConnect web app', async (request, argument, token) => {
                if (latestWebAppMessage) {
 ...
 */
async function addMessageToCursorChat(message) {
    ...
    try {
        ...
        const choice = await vscode.window.showInformationMessage(`💬 New message from web app: ${preview}`, 'Open Cursor Chat', 'Copy to Clipboard', 'View in Panel');
        switch (choice) {
            case 'Open Cursor Chat':
                ...
                try {
                    ...
                    vscode.window.showInformationMessage(`Message ready to paste: ${preview}`, 'Copy Message').then(copyChoice => {
 ...
 */
async function processPendingChatMessages() {
...
function showChatPanel() {
    ...
    if (chatPanel) {
        ...
        // Create new panel
        chatPanel = vscode.window.createWebviewPanel('liveideconnectChat', 'LiveIDEConnect Chat', column, {
            enableScripts: true,
...
function activate(context) {
    ...
    const connectCommand = vscode.commands.registerCommand('liveideconnect.connect', () => {
        ...
        if (!token || !sessionId) {
            vscode.window.showErrorMessage('LiveIDEConnect: Token and sessionId must be configured in settings first.', 'Open Settings').then(selection => {
                if (selection === 'Open Settings') {
                    vscode.commands.executeCommand('workbench.action.openSettings', 'liveideconnect');
                }
            });
    ...
    const checkStatusCommand = vscode.commands.registerCommand('liveideconnect.checkStatus', () => {
        const { token, sessionId, apiBase } = getConfig();
        const isConnected = ws && ws.readyState === ws_1.default.OPEN;
        ...
        outputChannel?.show();
        if (!isConnected) {
            vscode.window.showInformationMessage('LiveIDEConnect: Not connected. Check the output channel for details.', 'Connect Now', 'Open Settings').then(selection => {
    ...
    const sendMessageCommand = vscode.commands.registerCommand('liveideconnect.sendMessage', async () => {
        ...
        if (input && ws && ws.readyState === ws_1.default.OPEN) {
            const message = {
                type: 'agent_message',
                sessionId: getSessionId(),
                from: 'ide',
        ...
        else if (!ws || ws.readyState !== ws_1.default.OPEN) {
            vscode.window.showWarningMessage('Not connected. Please connect first.');
        }
    });
